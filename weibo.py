...
data_dir = "/data/wangao/weiboData/"
class Weibo(object):
    def __init__(self, config):
        ...
        if not isinstance(user_id_list, list):
            ...
            user_id_name = pd.read_csv(user_id_list)
            temp_id_list = user_id_name[start_user_index:end_user_index]['id'].values
    ...
    def write_user_info(self):
        ...
        file_dir = data_dir + "data/{}/".format(self.user_config["user_id"])
        if not os.path.isdir(file_dir):
            os.makedirs(file_dir)
        file_path = file_dir + "info.txt"
        with open(file_path, 'w') as f:
            for k, v in self.user.items():
                f.writelines("{}-{}\n".format(k, v))
        logger.info("user info write to {}".format(file_path))
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            if not os.path.isdir(file_dir):
                os.makedirs(file_dir)
            file_path = file_dir + os.sep +  "weibo." + type
            return file_path
        except Exception as e:
            logger.exception(e)
